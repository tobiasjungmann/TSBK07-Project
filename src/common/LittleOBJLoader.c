...
static void ParseLine(FILE *fp, char *line)
{
	int i, j;
	char c = '_';
...
void DisposeMtlList(Mtl **materials)
{
	if (materials != NULL)
...
static Mtl **ParseMTL(char *filename)
{
	...
	materials = (Mtl **)calloc(matcount+1, sizeof(MtlPtr));
	...
	while (!atFileEnd)
	{
		...
		{
			matcount++;
			materials[matcount-1] = (Mtl *)calloc(1, sizeof(Mtl));
...
char ParseOBJ(const char *filename, MeshPtr theMesh)
{
...
// Dispose if they exist!
	...
	gMtlNameList = (char **)calloc(theMesh->groupCount+1, sizeof(char *));
	theMesh->coordStarts = (int *)calloc(theMesh->groupCount+2, sizeof(int)); // Length found on last pass
	theMesh->groupCount = 0;
...
void GenerateNormals(Mesh* mesh)
{
	...
	if (mesh->vertices && !mesh->vertexNormals)
	{
		...
		mesh->vertexNormals = (vec3 *)calloc(mesh->vertexCount, sizeof(vec3));
...
Mesh **SplitToMeshes(Mesh *m)
{
	...
	Mesh **mm = (Mesh **)calloc(m->groupCount+2, sizeof(Mesh *));
	...
	for (int mi = 0; mi < m->groupCount; mi++) // For all sections
	{
		...
		mm[mi] = (Mesh *)calloc(1, sizeof(Mesh)); // allocate struct
...
void LOLError(const char *caller, const char *name)
{
...
void InitModel(Model *m, GLuint program, const char* vertexVariableName, const char* normalVariableName, const char* texCoordVariableName)
{
#ifdef __cplusplus
	InitModel(m, program, vertexVariableName, normalVariableName, texCoordVariableName, -1, -1);
}
...
void InitModel(Model *m, GLuint program, const char* vertexVariableName, const char* normalVariableName, const char* texCoordVariableName, GLint texUnit, GLint texReferenceID) {
	...
	if (m != NULL)
	{
		...
		if (vertexVariableName != NULL) // personal addon
		{
		...
		if ((m->texCoordArray != NULL)&&(texCoordVariableName != NULL))
		{
			...
			if (loc >= 0)
			{
				...
				#ifdef __cplusplus
				if (not m->material)
					m->material = (MtlPtr) calloc(1, sizeof(Mtl));
				m->material->texRefID = texReferenceID;
				m->material->texUnit = texUnit;
				#endif //__cplusplus
			}
			else
...
Model** LoadModelSet(const char* name)
{
	...
	int i, ii;
	for (i = 0; mm[i] != NULL; i++) {}
	Model **md = (Model **)calloc(i+1, sizeof(Model *));
