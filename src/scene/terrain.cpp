...
static void addToNormalArray(vec3 *normalarray, vec3 *vertexarray, int previousVertexIndex, int thisVertexIndex, int nextVertexIndex)
{
 ...
 */
static void addTriangle(GLuint *indexArray, vec3 *normalArray, vec3 *vertexArray, int first, int second, int third, int offset, int width, int x, int z)
{
...
namespace scn
{
      ...
      : key{name}
  {
    ...
    auto mdl { rmgr.getModel(key, Terrain::generateTerrain, texp) };
  ...
  {
    auto mdl = m_terrain.get();
    float x_difference = x - int(x);
    float z_difference = z - int(z);

    vec3 p_bottom_left = mdl->vertexArray[int(x) + (int(z) + 1) * m_width];
    vec3 p_top_right = mdl->vertexArray[((int(x) + 1) + int(z) * m_width)];

    vec3 startingPoint;
    vec3 v1, v2;
    if (x_difference + z_difference < 1.0)
    {
      startingPoint = mdl->vertexArray[(int(x) + int(z) * m_width)]; // p_top_left;
    ...
    {
      startingPoint = mdl->vertexArray[((int(x) + 1) + (int(z) + 1) * m_width)];
